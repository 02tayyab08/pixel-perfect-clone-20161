...
**Part 2 updated:** 2026-07-22 (aligned with Part 1 — outage confirmed, public hardening, genai 2.11.0, IFZA corpus, next-session priorities).

---

# SECTION 8 — NEXT STEPS

## 8.0 PRODUCTION OUTAGE — RESOLVED AND CONFIRMED (2026-07-21)
...
**Permanent removal (confirmed):** commit **`4eeaf86`** deleted the probe source files and the `mammoth` / `pdf-parse` dependencies. Standing decision: the probe is **not** returning in its current form (full outage + non-discriminative calibration). The `process-documents.ts` L143–147 comment block is retained as historical documentation.
...
**Theories cleared for the outage:** recovery on then-current bun `@google/genai` **2.10.0** with bare `"crypto"` and `Buffer` still on the boot graph cleared both the SDK-skew and `nodejs_compat` theories **as explanations for the 500**. Separately, `5dc2837` later pinned bun to **2.11.0** because 2.10.0's stream parser fails on multi-chunk grounded responses (post-outage defect; see §7.1).
...
## 8.1 Ordered plan — next session (priority)

1. **Widget grounded-answer failure — `"Incomplete JSON segment at the end"` (P0).** Staff `/api/query` works with `fileSearch` alone. Widget `/api/public/widget-query` adds `capture_lead` + `toolConfig.includeServerSideToolInvocations`. **Decisive untested experiment:** strip those two from a widget request and retry a grounded question. That single A/B also answers §6.5's compose question (does `fileSearch` + `capture_lead` work on flash-lite, or is two-pass required?). Do not redesign until that result is in.
2. **Multi-figure citations lighting only one grounded row (P1).** Before any highlighter code change, inspect live `groundingSupports` count / segment texts for the answer — may be one support covering multiple fees, not a Tier 1 bug.
3. **Staff side-panel layout regression (P2, cosmetic).** [UNCERTAIN] exact repro steps / commit that introduced it — fix only after the two items above or with a clear screenshot repro.
4. **De-specification audit** — strip IFZA-specific fee names out of the RUNTIME prompt, keep them in the eval only. Required before onboarding any non-IFZA customer. See §7.3.
5. **Review / merge `widget-persona-wip`** — see BRANCH STATE. Not deployed; `main` is still what §6.3 / §6.5 describe unless a sentence says otherwise.
6. **Storage webhook / XOR verification re-run** — polish / baseline hygiene after P0–P1.

### Shipped since prior handoff (do not re-do)

- Public widget hardening `26254e1` — history gated + POST; both endpoints take `slug` (Part 1 §4).
- `@google/genai` **2.11.0** in `bun.lock` (`5dc2837`).
- IFZA clean markdown **confirmed serving**; flattened PDF deleted from `recoupr-3` (Part 1 §2 corpus).
- Outage permanent removal `ad75380` → `4eeaf86` (this §8.0).
...
**`main` remains the deployed / production code path.** Everything in §6.3 and §6.5 describes **`main`**, not this branch, unless a sentence explicitly says otherwise.
...
- [UNCERTAIN] whether this branch's `bun.lock` / dep tree still needs a rebase onto `main` after `4eeaf86` (mammoth/pdf-parse delete) and `5dc2837` (genai 2.11.0) — rebase/reconcile before merge
...
**Architecture (built on `main`; compose with File Search still unproven end-to-end):**
...
### Live failure on `main` (blocks both grounding and lead capture)

**Symptom:** widget grounded answers fail with a stream/parse error along the lines of **`"Incomplete JSON segment at the end"`**. Staff path answering the same corpus with **`fileSearch` alone** works.

**Likely delta (do not treat as proven):** widget adds `capture_lead` + `includeServerSideToolInvocations`; staff does not.

**Decisive experiment (next session #1 — not yet run):** temporarily strip `capture_lead` and `includeServerSideToolInvocations` from the widget request, retry a known in-docs question.  
- If it succeeds → compose is the problem; resume two-pass design (§ below) or find a supported single-call config.  
- If it still fails → look elsewhere (SDK stream, persona branch vs `main`, etc.).  
This experiment **also** answers the long-standing §6.5 compose question.

The prepared instrument-and-observe prompt below remains useful **after** the strip experiment (or if strip already proves compose is fine and `functionCall` still never fires).
...
- **`IFZA_Complete_Schedule_of_Fees_2024.md` — THE CURRENT ONE.** Lossless, all 15 pages. **CONFIRMED serving** on `recoupr-3`; old flattened PDF deleted (Part 1 §2).
...
1. **Widget `"Incomplete JSON segment at the end"` / does `fileSearch` + `capture_lead` compose on flash-lite?** P0. Staff works with `fileSearch` alone; widget adds capture tool + `includeServerSideToolInvocations`. Decisive strip experiment not yet run — see §6.5 and §8.1.
2. ~~**What is actually causing the production 500?**~~ **RESOLVED AND CONFIRMED** — probe/`mammoth`/`pdf-parse` boot crash; `ad75380` then `4eeaf86`. See §8.0.
...
7. **`@google/genai` / dual lockfiles:** bun.lock pinned to **2.11.0** (`5dc2837`; needed because 2.10.0 failed multi-chunk grounded streams). **Other package drift between `bun.lock` and `package-lock.json` is still unreconciled** — standardize on bun going forward.
8. **Merge decision for `widget-persona-wip` (`84eb79e`):** see BRANCH STATE. Not on `main`; not deployed.
9. **Multi-figure citation lighting only one row:** check `groundingSupports` count before changing highlighter (§8.1 #2).
10. **Staff side-panel layout regression:** cosmetic; repro not fully written down (§8.1 #3).

## 7.1b Public widget hardening — residuals (`26254e1`)

Shipped: history is POST + gated (`is_active`, Origin, IP); both public endpoints take `slug`; resolved `org.id` for all privileged uses.

**Accepted residuals:**
- `org.id` still reaches the client via embed loader + `salni.eur.${org.id}` / `salni.locale.${org.id}` (do not rename — orphans visitors).
- Slug endpoints are enumerable by design.

**Deferred:** HMAC or httpOnly-cookie binding of `endUserRef` to the org.
...
**Routes / server functions (was absent from original handoff; status as of 2026-07-22):**
- `/api/public/widget-history` — **hardened** (`26254e1`): POST + slug + gates; see Part 1 §4 and §7.1b.
- Still lightly documented elsewhere: `/embed-loader.js`, `/api/session`, `/sitemap.xml`, `/auth/sign-in`, `/auth/sign-up`, `/_authenticated/onboarding`, `/` landing.
...
- **`bun.lock` AND `package-lock.json` both exist in the repo.** Lovable builds with **bun**; Cursor historically used **npm**. Any dependency added via npm lands in `package-lock.json` but NOT `bun.lock`, so Lovable's build cannot resolve it (hit with `react-markdown` / `remark-gfm` until `c6dfbf4`). **`@google/genai` is now 2.11.0 in bun.lock** (`5dc2837`); **other lockfile drift is still unreconciled.** **Recommendation: standardize on bun.**
...
- **Citation over-highlighting within the correct chunk** — a figure shared across rows (e.g. `1,000`) can light multiple rows. Accepted deliberately: over-highlighting within the right chunk is the safe direction; chasing it risks regressing to sparse highlights. (Separate from multi-figure / single-support questions — see §8.1 #2.)
- **On `main`, the widget still shows citation chips** (visitor citation removal lives on `widget-persona-wip` only). Deliberate asymmetry until that branch merges: staff has side panel; widget chips without panel on `main`.
